  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setGameState((prev) => {
        ...
        const player = newState.players.find((p) => p.id === selectedPlayer)
        if (player && !currentAction) {
          ...
          if (isWalkable(newState.map, newPos)) {
    ...
    return () => clearInterval(interval)
  }, [selectedPlayer, currentAction, gameState.gameOver])

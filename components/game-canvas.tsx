...
  startReport,
  completeReport,
  canUnblock, // <--- Mantenido del código 1
  startUnblock, // <--- Mantenido del código 1
  completeUnblock, // <--- Mantenido del código 1
  updateGameState,
} from "@/lib/game-actions"
import { EndGameScreen } from "@/components/end-game-screen" // <--- Mantenido
import { RecyclingMinigame } from "@/components/recycling-minigame" // <--- Mantenido
...
    gameOver: false,
    winner: null,
    criticalBlockActive: false, // <--- Mantenido del código 1
  })

  const [currentAction, setCurrentAction] = useState<Action | null>(null)
  const [selectedPlayer, setSelectedPlayer] = useState<string>("player1")
  const [showMinigame, setShowMinigame] = useState(false) // <--- Mantenido
...
  // Función resetGame (del código 1)
  const resetGame = () => {
    setGameState({
      map: GAME_MAP,
      players: [
...
    // Draw map (con sprites de edificio)
...
  	// Render Trash Cans (del código 1)
...
      // Selected indicator (adaptado para sprites)
...
      // Health bar (Y ajustada para sprites)
      const barWidth = 24
      const barHeight = 4
      const barX = centerX - barWidth / 2
      const barY = centerY - 20 // Sube la barra
...
    // Draw action progress (del código 1)
    if (currentAction) {
...
  	// Render Critical Block (del código 1)
...
  // Handle keyboard input (fusionado y CORREGIDO)
  useEffect(() => {
...
      // --- LÓGICA DE ACCIONES (del código 1) ---
...
  	}
  }, [gameState, selectedPlayer, currentAction]) // Dependencias del código 1

  // Action progress interval (fusionado)
  useEffect(() => {
...
        	} else if (updated.type === "unblock") { // <--- Lógica de Unblock (del código 1)
...
  // Game state update interval (del código 1)
  useEffect(() => {
  	if (gameState.gameOver) return
...
  // --- ELIMINADO: El "Game loop" (useEffect con setInterval 100) del código 1 ---
  // El movimiento ahora se maneja en handleKeyDown.

  // handleMinigameComplete (del código 1)
...
  // Variables para el JSX (del código 1)
  const player = gameState.players.find((p) => p.id === selectedPlayer)
...
  const canDoUnblock = player?.role === "cleaner" && canUnblock(gameState, selectedPlayer) // <--- Mantenido
...
          	</div>
          	<div className={canDoClean && !canDoUnblock ? "text-green-600 font-semibold" : ""}>
            	{!canDoUnblock && `Press SPACE to clean trash can ${canDoClean ? "(available!)" : ""}`}
          	</div>
